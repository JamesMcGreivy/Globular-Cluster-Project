...
vector<bool> select2(System &sys, double dt, double &tmin) {
	...
	for (int i = 0; i < size; i ++)
	{
		average_velocity += magnitude( (1/sys[i].mass) * sys[i].momentum );
	}

	average_velocity = average_velocity/ size;

	for (int i = 0; i < size; i ++) {
		
		
		ideal_dt = average_velocity / (2 + magnitude ( (1/sys[i].mass) * sys[i].momentum) );
...
void output_position(System sys, int end_time, string name, double _dt, string method) {
	...
	while (t <= end_time) {
		...
		for (int i = 0; i < sys.size(); i ++)
		{
			//cout << sys[i].position << endl;
			//cout << (1/sys[i].mass) * sys[i].momentum << endl;
		}
		//cout << endl;
