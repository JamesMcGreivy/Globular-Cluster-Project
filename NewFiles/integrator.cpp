...
namespace LorentzForceIntegrator
{
	...
	private:
		...
		//The initial substep where electric and magnetic fields
		//are treated as roughly constant over the time-step
		...
		//Finds the g_values based on the initial conditions and the current b_values
		//but uses the "initial substep" method as the guess for x(h) and v(h)
	...
	public:
		...
		void step()
		{
			...
			for (int i = 0; i < num_particles; i++)
			{
			    ...
			    double t_0 = tan(qmdt)*B_0;
			    double t_1 = tan(qmdt)*B_1;
			    double t_2 = tan(qmdt)*B_2;
