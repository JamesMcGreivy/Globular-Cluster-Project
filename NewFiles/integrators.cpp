...
namespace LorentzForceIntegrator
{
	...
	private:
		...
		void init_substep(double h, double dt)
		{
		...
		Eigen::Matrix<double,8,3> init_b_values;
	...
	public:
		...
		void step()
		{
			...
			//std::cout << b_values[0] - old_b_values[0] << std::endl << std::endl;
			if (count > 6 and (global_error <= 1e-15 or count >= 30)) //global_error < 10e-11 or count > 10)
			{	
				double max_b8 = 0;
				for (int i = 0; i < num_particles; i++)
				{
